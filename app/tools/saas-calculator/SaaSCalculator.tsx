...
export function SaaSCalculator() {
    ...
    const generateMarkdown = () => {
        ...
        const month6 = growthData[5]
        const month12 = growthData[11]

        let md = `## SaaS Simulation Report (${dateStr})\n\n`
        md += `### 📊 Key Metrics\n`
        md += `- **MRR:** ${formatCurrency(totalMRR, lang)}\n`
        md += `- **ARR:** ${formatCompactCurrency(totalARR, lang)}\n`
        md += `- **Profit Margin:** ${profitRate.toFixed(1)}%\n`
        md += `- **ARPU:** ${formatCurrency(arpu, lang)}\n\n`

        md += `### 📈 12-Month Forecast\n`
        md += `| Month | Users | MRR | Cumulative Revenue |\n`
        md += `| :--- | ---: | ---: | ---: |\n`
        md += `| 1 | ${growthData[0]?.ユーザー数.toLocaleString()} | ${formatCurrency(growthData[0]?.総MRR || 0, lang)} | ${formatCurrency(growthData[0]?.累積純売上 || 0, lang)} |\n`
        md += `| 6 | ${month6?.ユーザー数.toLocaleString()} | ${formatCurrency(month6?.総MRR || 0, lang)} | ${formatCurrency(month6?.累積純売上 || 0, lang)} |\n`
        md += `| 12 | ${month12?.ユーザー数.toLocaleString()} | ${formatCurrency(month12?.総MRR || 0, lang)} | ${formatCurrency(month12?.累積純売上 || 0, lang)} |\n\n`

        md += `### ⚙️ Settings\n`
        md += `- **Plans:** ${freePlanEnabled ? 'Free / ' : ''}Standard (${formatCurrency(numStarterPlanPrice, lang)}) / Pro (${formatCurrency(numProPlanPrice, lang)})\n`
        md += `- **Growth Rate:** +${numMonthlyGrowthRate}% / Churn: ${numChurnRate}%\n`
        md += `- **First Month Free:** ${firstMonthFree ? 'Yes' : 'No'}\n\n`

        md += `> Generated by [kozo.dev SaaS Calculator](https://kozo.dev/tools/saas-calculator)`
    ...
    const generateYAML = () => {
        ...
        const plans: Record<string, { price: number; distribution: number }> = {}
        if (freePlanEnabled && effectiveFreePercent > 0) {
            plans.free = { price: numFreePlanPrice, distribution: effectiveFreePercent / 100 }
        }
        plans.standard = { price: numStarterPlanPrice, distribution: effectiveStarterPercent / 100 }
        plans.pro = { price: numProPlanPrice, distribution: effectiveProPercent / 100 }

        const yaml = `project: "SaaS Simulation"
date: ${dateStr}
currency: ${lang === 'ja' ? 'JPY' : 'USD'}

inputs:
  total_users: ${numTotalUsers}
  plans:
${Object.entries(plans).map(([name, p]) => `    ${name}: { price: ${p.price}, distribution: ${p.distribution.toFixed(2)} }`).join('\n')}
  growth:
    monthly_rate: ${(numMonthlyGrowthRate / 100).toFixed(2)}
    churn_rate: ${(numChurnRate / 100).toFixed(2)}
  options:
...
metrics:
  ...
  profit_margin: ${(profitRate / 100).toFixed(3)}
  arpu: ${Math.round(arpu)}
  arppu: ${Math.round(arppu)}

forecast_12_months:
  end_users: ${lastMonth?.ユーザー数 || 0}
  end_mrr: ${lastMonth?.純MRR || 0}
  cumulative_revenue: ${lastMonth?.累積純売上 || 0}
`
        return yaml
    }
